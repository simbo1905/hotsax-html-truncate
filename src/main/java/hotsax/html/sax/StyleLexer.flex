...
/* works only only inside of style tags */
<YYINITIAL> {
	"<!--" {}	
	"-->" {}

	/* any close of this style tag returns the collected text, followed by tag stuff */
	"</style>" {
...
/* XHTML style blocks might have all style code inside a CDATA element */	
<IGNORE_CDATA> {
    "]]>" {
        yybegin(YYINITIAL);
